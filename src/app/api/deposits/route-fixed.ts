...
export async function GET(request: NextRequest) {
  try {
    // For now, bypass auth since RLS is disabled and we know deposits exist
    // TODO: Fix server-side auth later
    const { searchParams } = new URL(request.url)
    const page = parseInt(searchParams.get('page') || '1')
    const limit = parseInt(searchParams.get('limit') || '10')
    const offset = (page - 1) * limit

    // Get all deposits for now (since RLS is disabled, this is safe)
...
export async function POST(request: NextRequest) {
  try {
    // Try to get user from request headers first

-- Create table to track daily income transactions
CREATE TABLE IF NOT EXISTS income_transactions (
    ...
    is_final_collection BOOLEAN DEFAULT FALSE,
...
CREATE POLICY "Users can view own income transactions" ON income_transactions
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Admins can view all income transactions" ON income_transactions
    FOR ALL USING (
        EXISTS (
...
-- Update the collect_daily_income function to record transactions

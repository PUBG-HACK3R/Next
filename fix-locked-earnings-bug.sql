-- Fix the locked earnings bug where completing one investment transfers all locked earnings
-- Instead of transferring all earned_balance, only transfer earnings from the completed investment

-- Create a new function to transfer only specific investment earnings
...
-- Update the collect_daily_income function to use the new logic
...
BEGIN
  ...
  IF is_final_collection THEN
    ...
    -- ✅ FIX: Only transfer earnings from THIS completed investment, not all earned balance
...
COMMENT ON FUNCTION transfer_investment_earnings_to_main(UUID, INTEGER) IS 'Transfers only earnings from a specific completed investment to main balance, keeping other locked earnings intact';
COMMENT ON FUNCTION collect_daily_income(INTEGER, UUID) IS 'Fixed version - only transfers earnings from completed investment, not all locked earnings';

-- Verify the functions were updated
SELECT 'Locked earnings bug fix applied successfully' AS status;

-- Fix the collect_daily_income function to resolve EXTRACT error
...
BEGIN
    ...
    -- Calculate days since last collection (FIXED)
    IF investment_record.last_income_collection_date IS NULL THEN
        -- First collection - calculate days since start_date
        days_since_last_collection := (CURRENT_DATE - investment_record.start_date::DATE) + 1;
    ELSE
